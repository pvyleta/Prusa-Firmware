...
- TMC2130_STEALTH_Z was disabled by Clixter, I re-enabled it - should be OK to keep it enabled
- TMC2130_DEDGE_STEPPING was removed by Guykuo - it almost seems like ommitment, as it was not mentioned anywhere in the original prusa thread (TODO - mention the prusa3d forum thread)
- since XY_AXIS_FEEDRATE is being used at multiple places for X and Y axes at the same time, it makes sense to have identical motors on X and Y (Though it would likely work anyways)
- TMC_SET_CHOP_[X|Y|Z] can be used to set choper parameters instead of M919-M922 introduced originally by Guykuo
- Going back to stock FW: Before you reflash, just set all 4 steppers to 'Prusa'. This erases all custom config in EEPROM and you can safely load stock FW. Note: clearing happens when Prusa stepper is selected, so in case you have custom chopper config on Prusa stepper, you need to click through the other steppers to ensure errasure.
- If at some point you had FW newer than 3.14.1, you need to erase the eeprom, as there may be some leftover settings left at the addresses used by this FW
